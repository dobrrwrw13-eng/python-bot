...
# НАЛАШТУВАННЯ
# =======================
API_TOKEN = "7911231063:AAFE0JM0SAl9kZsiTdthqlLM6-mZF3SxxfY"
...
EMAIL_CONFIG = {
    ...
    'email': os.getenv('EMAIL_ADDRESS', 'no-reply@bgpk-liceum.site'),
    'password': os.getenv('EMAIL_PASSWORD', 'Viot561890!'),
...
def db_insert_schedule(class_name: str, day_name: str, lesson_number: int, subject: str, teacher: str, start_time: str, end_time: str) -> None:
    DB.execute(
        """
        INSERT OR REPLACE INTO schedules (class_name, day_name, lesson_number, subject, teacher, start_time, end_time)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
...
def db_get_schedule_for_user_today(phone_norm: str) -> list:
    ...
    cur = DB.execute(
        "SELECT * FROM schedules WHERE class_name = ? AND day_name = ? ORDER BY lesson_number",
        (class_name, day_ua),
    )
...
# =======================
# SCHEDULES удален - данные загружаются из коллекции schedules в базе данных
...
async def send_announcement_to_all(announcement_text: str, file_path: str, admin_tg_id: int):
    ...
    try:
        ...
        inline_kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="📞 Зв'язатися з адміністратором", url=f"tg://user?id={admin_tg_id}")]
            ]
        )
...
async def handle_message(message: types.Message, state: FSMContext):
    ...
    if current_state == Reg.waiting_for_phone.state:
        ...
        if user_by_phone:
            ...
            await message.answer(f"Ваше ФІО: {user_by_phone['fio']}?", reply_markup=kb_yes_no())
            await state.set_state(Reg.confirm_found_fio)
            return

        await message.answer(
            "Вас не було знайдено.\n\n"
            "Напишіть, будь ласка, своє ПІБ.\n\n"
    ...
    if current_state == Reg.confirm_found_fio.state:
        ...
        await message.answer("Введіть вірне ФІО:", reply_markup=ReplyKeyboardRemove())
        await state.set_state(Reg.input_fio)
        return
    ...
    if current_state == Reg.input_fio.state:
        ...
        if not is_valid_fio(fio):
            await message.answer("Будь ласка, введіть ФІО у форматі: Прізвище Ім'я По батькові.\nПриклад: Іванов Іван Іванович")
            return

        await state.update_data(fio=fio)
        await message.answer(f"Ваше ФІО «{fio}» вірно?", reply_markup=kb_yes_no())
